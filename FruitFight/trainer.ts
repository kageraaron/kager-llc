...
class NeuralSimulator {
  ...
  runGame(neuralWeights: number[], phase: number): { reward: number, won: boolean } {
    const neuralIdx = Math.floor(Math.random() * NUM_PLAYERS);
    
    // Select opponent types based on phase
    const opponentTypes: number[] = [];
    for (let i = 0; i < NUM_PLAYERS - 1; i++) {
      if (phase === 1) {
        ...
        const r = Math.random();
        if (r < 0.2) opponentTypes.push(Math.floor(Math.random() * 3));
        else if (r < 0.5) opponentTypes.push(3);
        else if (this.pastSelfPool.length > 0) opponentTypes.push(4);
        else opponentTypes.push(3);
      }
    }
    ...
    while (!state.isGameOver && turns < 1000) {
      ...
      if (state.activePlayerIndex === neuralIdx) {
        ...
        if (oppSlot > neuralIdx) oppSlot--; // Shift back to get opponentTypes index
        const type = opponentTypes[oppSlot];
        
        switch(type) {
          case 0: action = HeuristicBots.getConservativeAction(state); break;
          case 1: action = HeuristicBots.getAggressiveAction(state); break;
          case 2: action = HeuristicBots.getMathBotAction(state); break;
          case 3: action = AdvancedAI.getAction(state, DEFAULT_AI_PARAMS); break;
          case 4: 
  ...
  applyAction(state: GameState, action: string): GameState {
    ...
    if (action === 'STEAL') {
      ...
      for (const fp of fromPlayers) {
        ...
        while (j--) { if (otherDisplay[j] === card) { player.display.push(card); otherDisplay.splice(j, 1); } }
      }
    ...
    if (action === 'DRAW') {
      ...
      if (fromPlayers.length > 0) { state.pendingSteal = { card, fromPlayers }; return state; }
      player.display.push(card);
      if (state.deck.length === 0) return this.endGame(state);
      return state;
    }
    ...
    for (let g = 0; g < gens; g++) {
      ...
      if (g % 5 === 0 || g === gens - 1) {
        ...
        if (phase === 2 && g % 10 === 0) {
            ...
            if (sim.pastSelfPool.length > 20) sim.pastSelfPool.shift(); // Keep last 20
        }
      }

      // Evolution
      const nextPop: Genome[] = [];
      for (let i = 0; i < ELITISM_COUNT; i++) nextPop.push({ weights: [...population[i].weights], fitness: 0 });
