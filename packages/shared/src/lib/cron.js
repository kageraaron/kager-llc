/**
 * Parses a single cron field into an array of valid integers.
 */
...
/**
 * Describes a cron expression in human-readable language.
 */
 ...
 */
export function nextCronRuns(parts, count = 5) {
    ...
    // Per cron spec: when both DOM and DOW are restricted (non-*), either match satisfies the day.
    ...
    for (let i = 0; i < limit && runs.length < count; i++) {
        ...
        if (domRestricted && dowRestricted) {
            dayMatch = domVals.includes(cursor.getDate()) || dowVals.includes(cursor.getDay());
        } else {
            dayMatch = domVals.includes(cursor.getDate()) && dowVals.includes(cursor.getDay());
        }
