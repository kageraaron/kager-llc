...
- Find a way to programatically generate CENTERS: dict[str, dict[str, list[tuple[str, int]]]] = ... (line 62 of src/word_nexus_generator.py). This has proven to be the superior method for generating boards, but was hand-crafted. We need to find a way to expand this.
...
**Strengths**
- Mechanic is original and shareable. "I got it in 3 guesses, what's yours?" is the Wordle hook, and the `BRAIN + TENNIS = STROKE` aha moment is genuinely satisfying.
...
2. **Practice Mode** A way for serious players to scroll through all previous puzzles to improve their skills and have fun, and to stay on the page longer.
...
3. **Domain + simple landing page.** `dailywordnexus.com` or similar. Static deploy on Cloudflare Pages / Netlify (free). Open Graph tags so the share card previews nicely on iMessage, Twitter, Discord.
4. **Google Ads Campaign** configure an intelligent google ads campaign / script to run daily that assesses successful campaigns and ad words to reallocate the daily budget and continuously improve, using A/B test, etc.
5. **Email list ("Today's puzzle in your inbox")** — Enter to be on the mailing list below the daily game board. Becomes the single most valuable distribution asset you own. Sponsor slot at the bottom is sellable once list > 5k.
